...
Page({
  ...
  */
  ...
  onLoad: function (options) {
    //创建 map 上下文 MapContext 对象。mapContext 通过 id 跟一个 <map> 组件绑定，操作对应的 <map> 组件。
    this.mapCtx = wx.createMapContext('map');

    // 实例化API核心类
    qqmapsdk = new QQMapWX({
      key: 'KIYBZ-Y3FHI - TT5G3 - 5GOIM-4HXGZ-OZBXW'
    });
  ...
  onReady: function () {
    ...
    wx.getLocation({
      success: function(res) {
        ...
        wx.request({
          url: "http://192.168.11.211/kafka/track",
          data: {
            time: new Date(),
            openid: openid,
            lat: lat,
